            ...
            do
                ...
                do
                    ...
                    if [ ${LAMBDA} == 0 ]; then
                        ...
                        DIRECTORY=output/${DATASET}/nlab_${NLAB}/${BASELINE}/${IMBALANCE}/ME/shannon_${LAMBDA}/batchsize_${BATCH_SIZE}/${TRAINER}/${NET}/${T}/seed${SEED}
                    fi

                    if [ -d "$DIRECTORY" ]; then
                        if [ -d "$DIRECTORY/C" ] && [ -d "$DIRECTORY/G" ]; then
                            echo "Config already trained: $IMBALANCE $BATCH_SIZE $SETUP $SEED $LAMBDA"
                        else
                            ...
                            jq ". += [{\"imbalance\": \"${IMBALANCE}\", \"setup\": ${SETUP}, \"seed\": ${SEED}, \"lambda\": ${LAMBDA}}]" ${NEED_TRAINING_FILE} > tmp.json && mv tmp.json ${NEED_TRAINING_FILE}
                        fi
                    else
                        jq ". += [{\"imbalance\": \"${IMBALANCE}\", \"setup\": ${SETUP}, \"seed\": ${SEED}, \"lambda\": ${LAMBDA}}]" ${NEED_TRAINING_FILE} > tmp.json && mv tmp.json ${NEED_TRAINING_FILE}
                    fi
...
while true; do
    ...
    for row in $(jq -c '.[]' ${NEED_TRAINING_FILE}); do
        ...
        echo "Training config: imbalance=${IMBALANCE}, setup=${SETUP}, seed=${SEED}, lambda=${LAMBDA}"
